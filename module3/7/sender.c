...
int main() {
    ...
    char send_queue_name[] = "/send_queue";
    char receive_queue_name[] = "/receive_queue";
    ...
    while (1) {
        ...
        if (mq_send(send_desc, buffer, strlen(buffer), 1) == -1) {
            perror("mq_send");
            continue;
        }
        ...
        if (priority == TERMINATE_PRIORITY && strcmp(buffer, "TERMINATE") == 0) {
            printf("Получатель запросил завершение обмена.\n");
            break;
        }
